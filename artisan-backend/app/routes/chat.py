...
@router.post("/message/new", response_model=List[MessageResponse])
async def create_message(message: MessageCreate, db: Session = Depends(get_db)):
    try:
        response = []  # Response for the user
        ...
        if not message.is_bot:
            chatResponse = get_chat_response(message.content)
            bot_message = Message(
                user_id=message.user_id, content=chatResponse, is_bot=True
            )
            db.add(bot_message)
            db.commit()
...
@router.delete(
    "/messages/{user_id}/{message_id}", status_code=status.HTTP_204_NO_CONTENT
)
async def delete_user_messages(
    user_id: int, message_id: int, db: Session = Depends(get_db)
):
    try:
        tobeDeletedMessage = (
            db.query(Message)
            .filter(and_(Message.user_id == user_id, Message.id == message_id))
            .first()
        )
